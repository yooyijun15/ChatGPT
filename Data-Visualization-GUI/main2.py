...
def plot_temperatures():
    ...
    # Create a line or bar plot of the temperature data, depending on user selection
    graph_type = graph_type_selection.get()
    if graph_type == 'line':
...
# Create the graph type selection widgets
graph_type_label = tk.Label(window, text='Graph Type:')
graph_type_label.grid(row=3, column=0, padx=5, pady=5)

graph_type_selection = tk.StringVar()
graph_type_selection.set('line')

line_radio = tk.Radiobutton(window, text='Line', variable=graph_type_selection, value='line')
line_radio.grid(row=3, column=1, padx=5, pady=5)

bar_radio = tk.Radiobutton(window, text='Bar', variable=graph_type_selection, value='bar')
bar_radio.grid(row=3, column=2, padx=5, pady=5)
